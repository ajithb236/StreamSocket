...
def setup_database():
    try:
        ...
        if not cursor.fetchone():
            ...
            conn.commit()
            print("[+] Default user seeded successfully into the database.")
        else:
            print(f"[+] Default user '{test_user}' already exists in the database. Skipping injection.")

        print("[+] Database setup complete! You can now start the TCP server.")
